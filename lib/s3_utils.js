...
S3Utils = function (bucket) {
    ...
    this.generateTempUrlPair = function (expDate, callback) {
        privateGenerateListUrlPairs([],1,expDate,function(urlPairs){
            callback(urlPairs[0]);
        });
    };
    ...
    function privateGenerateListUrlPairs (urlPairs, pairCount, expDate, callback){
        if (null == urlPairs)
            urlPairs = [];
        var urlPair = {};
        var key = crypto.createHash('sha1').update(new Date().getTime().toString() + Math.random().toString()).digest('hex').toString();
        key = qs.escape(key);
        console.log('requesting url pair for', key);
        urlPair[S3_KEY] = key;
        var putParams;
        if (!!expDate)
            putParams = {Bucket: bucket, Key: key, ACL: "public-read", ContentType: "application/octet-stream", Expires: expDate };
        else
            putParams = {Bucket: bucket, Key: key, ACL: "public-read", ContentType: "application/octet-stream" };
        ...
        s3.getSignedUrl('putObject', putParams, function (err, url) {
            ...
            urlPair[S3_EXPIRES] = expDate;
            urlPair[S3_PUT_URL] = url;
            urlPair[S3_GET_URL] = "https://" + bucket + ".s3.amazonaws.com/" + qs.escape(key);
